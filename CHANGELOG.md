...
- **hook-development**: `SessionStart`, `Setup`, and `SubagentStart` are command-only (no conversation context for prompt or agent hooks); `SessionStart` matchers include `fork`; `SessionEnd` reasons are `clear`, `resume`, `logout`, `prompt_input_exit`, `other`; `permission_mode` is an optional string carrying `default`, `acceptEdits`, `bypassPermissions`, `plan`, `dontAsk`, or `auto`; the hook-type legend includes `mcp_tool`
...
- **agent-development**: `permissionMode` values are `default`, `acceptEdits`, `dontAsk`, `bypassPermissions`, `plan`, `auto`; `delegate` is not a permission mode
- **plugin-structure**: agent examples use the `name`/`description`/`model`/`color` frontmatter instead of the older `capabilities` field; command frontmatter has no `name` field; the TUI command is `/plugin`
- **command-development**: `check-frontmatter.sh` accepts `disallowed-tools`
...
- **hook-development**: Documented PreModelSwitch and PostModelSwitch hook events (30th and 31st events) for model switching control and observability — PreModelSwitch can block, confirm, or annotate; PostModelSwitch is observability-only (CC 2.1.251)
- **plugin-structure**: Documented Remote Control subagent tool call streaming — live streaming of subagent tool calls to Remote Control clients extends `--forward-subagent-text` capability (CC 2.1.251)
- **plugin-structure**: Documented plugin path traversal security fix — installation now rejects plugins with `..` path segments or symlinks escaping the plugin root (CC 2.1.251)
- **hook-development**: Documented symlink vulnerability fix in Read/Write/Edit tools — stricter symlink handling rejects symlinks resolving outside sandbox paths (CC 2.1.251)
- **hook-development**: Documented Grep/Glob symlink deny rule fix — permission rules now correctly apply to symlinked paths (CC 2.1.251)
- **mcp-integration**: Documented schedule local MCP server limitation — local MCP servers cannot be attached to cloud routines; only Claude.ai connectors work (CC 2.1.251)

### Changed

- **skill-development**: Plugin Eval (`claude plugin eval`) and Skill Doctor (`/skill-doctor`) marked as removed from bundled prompts (CC 2.1.251)
- **skill-development**: Built-in skill patterns (Workflow authoring reference, Cost optimization skill, Admin API reference) marked as removed from bundled prompts as part of major documentation removal (CC 2.1.251)
...
- **plugin-structure**: Documented `defaultEnabled: false` manifest option for plugins requiring manual enablement (CC 2.1.154)
- **agent-development**: Documented background session temporary file path guidance (`$CLAUDE_JOB_DIR/tmp`) (CC 2.1.154)
- **agent-development**: Documented AskUserQuestion tightened usage guidance — ask only when blocked (CC 2.1.154)
- **hook-development**: Documented `$TMPDIR` sandbox clarification — same writable directory for sandboxed and unsandboxed commands (CC 2.1.154)
- **marketplace-structure**: Documented directory-aware plugin suggestions in Discover tab (CC 2.1.154)
- **mcp-integration**: Documented `CLAUDE_CODE_SESSION_ID` and `CLAUDECODE=1` environment variables for MCP servers (CC 2.1.154)
...
- **Loader-binding caveat for `${CLAUDE_PLUGIN_ROOT}` in frontmatter hooks.** Documented the observed runtime behavior: `${CLAUDE_PLUGIN_ROOT}` resolves only when an agent or skill file is loaded through plugin discovery; agents loaded via the `--agent` CLI flag from `.claude/agents/` see the variable unbound and the hook fails with "Hook command references CLAUDE_PLUGIN_ROOT but the hook is not associated with a plugin." Workaround is to use `${CLAUDE_PROJECT_DIR}` with a project-relative path. Surface became newly reachable in CC 2.1.116 (main-thread frontmatter hooks via `--agent`).
  - Caveat added to `references/hook-development/overview.md` (Scoped Hooks section + Environment Variables + Critical Gotchas), `references/hook-development/references/advanced.md`, `references/agent-development/overview.md` (hooks field), and `references/agent-development/references/advanced-agent-fields.md`
  - Cross-references upstream issues [#24529](https://github.com/anthropics/claude-code/issues/24529), [#27145](https://github.com/anthropics/claude-code/issues/27145), [#50357](https://github.com/anthropics/claude-code/issues/50357)
