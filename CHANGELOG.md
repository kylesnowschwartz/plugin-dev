...
- **hook-development**: prompt and agent hooks are unavailable on `SessionStart`, `Setup`, `SubagentStart` (no conversation context) and on `WorktreeCreate`, `WorktreeRemove` (run outside the REPL); HTTP hooks are unavailable on `SessionStart` and `Setup`; `mcp_tool` is accepted wherever `command` is. `validate-hook-schema.sh` and the CI component-validation allowlist follow the same matrix
