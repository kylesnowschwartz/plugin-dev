...
- **Executables (`bin/`, CC 2.1.91)** — files in `bin/` (compiled binaries or scripts with a shebang) can be invoked as bare commands from the Bash tool. Requires execute permissions (`chmod +x`) and platform-compatible binaries. Use to ship formatters, linters, converters, or standalone utilities.
  - **How it reaches the shell**: while the plugin is enabled, Claude Code appends `<plugin-root>/bin` to the end of `PATH` in the Bash tool's shell snapshot. The user's existing `PATH` comes first, so an executable of the same name already on `PATH` wins; give plugin executables distinctive names. On macOS and Linux, a plugin root whose path contains `:`, quotes, `$`, a backtick, a backslash, or a newline is left off `PATH`. When `/reload-plugins` changes the set of plugin `bin/` directories, the next Bash command rebuilds the snapshot.
  - **Scope**: the `PATH` change applies to the Bash tool only. Hooks and MCP servers run with Claude Code's own environment plus `CLAUDE_PLUGIN_ROOT` and related variables, not with the plugin's `bin/` on `PATH`, so reference executables explicitly from hook commands and MCP server configs: `${CLAUDE_PLUGIN_ROOT}/bin/<name>`.
  - **claude.ai organization distribution**: claude.ai rejects a plugin with a top-level `bin/` directory when it is distributed through organization settings (marketplace sync or direct upload). For that channel, put executables in `scripts/` and reference them as `${CLAUDE_PLUGIN_ROOT}/scripts/<name>`. See `../command-development/references/marketplace-considerations.md` ("Executables and claude.ai Organization Distribution").
...
| `references/headless-ci-mode.md` | Building or testing plugins for `claude -p` — what works headless, permission flags, structured output, system-prompt flags, session management, subagent forking (CC 2.1.121), `--forward-subagent-text` for stream-json subagent output (CC 2.1.211) |
