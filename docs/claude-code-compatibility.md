...
Last audited: Claude Code 2.1.150 (2026-05-25)
...
|---|---|---|---|
| v0.17.0 | 2.1.145-2.1.150 | 2026-05-25 | Workflow tool, pinned background sessions, hook `if` PowerShell fix, Stop/SubagentStop `background_tasks`/`session_crons` fields, `allowAllClaudeAiMcps` enterprise MCP, MCP pagination fix, `claude agents --json`, `/plugin` preview, multiple `Agent(...)` fix, `CLAUDE_CODE_SUBAGENT_MODEL` teammate scope |
