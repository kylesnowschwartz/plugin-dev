...
Last audited: Claude Code 2.1.154 (2026-05-28)
Plugin-dev version: 0.19.0

## Audit Log

| plugin-dev | CC version range | Date | Notes |
|---|---|---|---|
| v0.19.0 | 2.1.154 | 2026-05-28 | Opus 4.8 model support with high-effort defaults, `defaultEnabled: false` plugin manifest field, stdio MCP server `CLAUDE_CODE_SESSION_ID` + `CLAUDECODE=1` env vars, background session tmp directory change to `$CLAUDE_JOB_DIR/tmp`, AskUserQuestion tightened usage guidance |
